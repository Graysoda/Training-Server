...
public class RentalDao extends Database {
	...
	public List<Rental> getByStartDate(String date) {
	    String query = baseQuery + " WHERE r.rental_id IN (SELECT r.rental_id FROM sakila.rental r WHERE DATE_TRUNC('day', r.return_date) >= CAST('"+date+"' AS DATE) AND DATE_TRUNC('"+date+"' AS DATE) + CAST('1 DAY' AS INTERVAL)";
